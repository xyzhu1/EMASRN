...
class GFMRDB(nn.Module):
    ...
    def forward(self, input_feat, last_feats_list):
        ...
        if len(last_feats_list) == 0:#如果t==1
            for b in self.RDBs_list:#经过每一个RDB模块，网络中一共设置有7个该模块
                input_feat = b(input_feat)
                cur_feats_list.append(input_feat)
        else:
            for idx, b in enumerate(self.RDBs_list):#一共有7个RDB

                # refining the lowest-level features
                if idx < self.num_refine_feats:#这里的num_refine_feats设置为1，其实也还是在idx为0的地方将高级特征返回过来

                    select_feat = self.GFMs_list[0](torch.cat(last_feats_list, 1))
                    #实际上就是将上一个t的后四个特征图cat起来，然后卷积操作

                    input_feat = self.GFMs_list[1](torch.cat((select_feat, input_feat), 1))
                    #将Gate unit单元得到的输出与新的input_feat进行处理
        ...
        #这里的num_reroute_feats设置为4，即七个RDBs的最后4个特征作为下一个t的输入
...
class GMFN(nn.Module):
    ...
    def forward(self, lr_img):
        lr_img = self.sub_mean(lr_img)#干啥的
        up_lr_img = self.upsample(lr_img, scale_factor=self.upscale_factor, mode='bilinear', align_corners=False)
        #上采样，而且采用的不是pixshuffle

        init_feat = self.feat_in(self.conv_in(lr_img))#初始特征提取部分

        sr_imgs = []
        last_feats_list = []#最后特征的列表

        for _ in range(self.num_steps):#self.num_steps=2
