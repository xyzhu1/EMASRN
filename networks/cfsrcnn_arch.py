...
import networks.cfsrcnn_ops as ops
...
class CFSRCNN(nn.Module):
    def __init__(self):
        ...
        scale = 3  #kwargs.get("scale")  # value of scale is scale.
        multi_scale = False #kwargs.get("multi_scale")  # value of multi_scale is multi_scale in args.
        group =1 # kwargs.get("group", 1)  # if valule of group isn't given, group is 1.
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(in_channels=2 * features, out_channels=features, kernel_size=kernel_size1, padding=0, groups=1,
                      bias=False), nn.ReLU(inplace=True))
        ...
        self.conv34 = nn.Sequential(
            nn.Conv2d(in_channels=2 * features, out_channels=features, kernel_size=kernel_size, padding=1, groups=1,
                      bias=False))
        ...
        self.conv41 = nn.Sequential(
            nn.Conv2d(in_channels=features, out_channels=3, kernel_size=kernel_size, padding=padding, groups=groups,
                      bias=False))
